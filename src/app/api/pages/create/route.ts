...
export async function POST(request: NextRequest) {
  try {
    ...
    if (donatedDomainId) {
      ...
      isAvailable = await donatedDomainManager.checkSubdomainAvailability(
        donatedDomainId,
        subdomain,
      );
    ...
    try {
      ...
      deploymentId = await pageManager.createDeployment(
        pageRecord.id,
        files.length,
      );
      ...
      let repo;
      try {
        repo = await githubAPI.createRepository(
          repoName,
        ...
        throw new Error(
          `Failed to create GitHub repository: ${repoError.message}`,
        );
      }
      ...
      try {
        const uploadSuccess = await githubAPI.uploadFiles(
          repoName,
          fileUploads,
        );
        if (!uploadSuccess) {
          throw new Error(
            "File upload returned false - no files were uploaded successfully",
          );
        }
        ...
        throw new Error(
          `Failed to upload files to repository: ${uploadError.message}`,
        );
      }
      ...
      if (usingDonatedDomain && donatedDomainInfo) {
        ...
        const credentials =
          await donatedDomainManager.getDomainCredentials(donatedDomainId);
        if (!credentials) {
        ...
        dnsSuccess =
          await DonatedDomainCloudflare.createSubdomainForDonatedDomain(
            subdomain,
            actualDomain,
      ...
      await pageManager.updatePageStatus(pageRecord.id, "active", "deployed", {
      ...
      if (pageRecord) {
        await pageManager.updatePageStatus(pageRecord.id, "error", "failed", {
          metadata: {
            ...pageRecord.metadata,
