...
async function fetchWithHybridApproach(
...
async function tryGitHubAPI(
  ...
  try {
    ...
    console.log(
      "🎫 Using token type:",
      userAccessToken ? "User OAuth token" : "App token",
    );
    ...
    if (!response.ok) {
      console.warn(
        `⚠️ GitHub API response: ${response.status} ${response.statusText}`,
      );
      return [];
    }
...
async function scrapeGitHubProblemsPage(): Promise<string[]> {
  try {
    const pageUrl =
      "https://github.com/sriox-cloud/codeer_problems/tree/main/problems";
...
function parseMarkdownToProblem(
  content: string,
  filename: string,
): GitHubProblem | null {
  try {
        ...
        if (currentSection && sectionContent.trim()) {
          ...
          switch (sectionLower) {
            ...
            case "final answer":
              ...
              let cleanedSolution = sectionContent
                .trim()
