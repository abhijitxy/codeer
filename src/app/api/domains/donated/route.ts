  ...
  try {
    ...
    if (!domain_name || !cloudflare_zone_id || !cloudflare_api_token) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: domain_name, cloudflare_zone_id, cloudflare_api_token",
        },
        { status: 400 },
      );
    }
    ...
    if (!result) {
      return NextResponse.json(
        {
          error:
            "Failed to submit domain. Please check your Cloudflare credentials.",
        },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json({
      ...
      domain: safeResult,
      message:
        "Domain submitted successfully! It will be available for users after verification.",
    });
  } catch (error: any) {
    ...
    if (error.message.includes("Invalid Cloudflare credentials")) {
      return NextResponse.json(
        {
          error:
            "Invalid Cloudflare credentials. Please check your API token and Zone ID.",
        },
        { status: 400 },
      );
    }
