...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!contentsResponse.ok) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch repository contents", details: error },
        { status: 500 },
      );
    }
    ...
    console.log(
      "Contents:",
      contents.map((item: any) => ({ name: item.name, type: item.type })),
    );
        ...
        if (item.type === "file") {
          ...
          try {
            ...
            if (dirResponse.ok) {
              ...
              console.log(
                `Directory ${fullPath} has ${dirContents.length} items`,
              );
              const dirFiles = await fetchAllFiles(dirContents, fullPath);
              allFiles.push(...dirFiles);
            } else {
              console.error(
                `Failed to fetch directory ${fullPath}:`,
                dirResponse.status,
              );
            }
          } catch (error) {
