...
export default function CodeEditor({ session }: CodeEditorProps) {
  ...
  const createNewFile = () => {
    const newFileName =
      prompt("Enter file name:") || `file${openFiles.length + 1}.py`;
  ...
  const saveCode = async () => {
    ...
    try {
      ...
      if (response.ok) {
        alert(
          `File saved to GitHub successfully! Repository: ${result.repository}`,
        );
      } else {
        console.error("GitHub save error:", result);
        alert(
          `Failed to save file to GitHub: ${result.error || "Unknown error"}`,
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (loadedFiles.length > 0) {
          ...
          alert(
            `Successfully loaded ${loadedFiles.length} files from repository: ${result.repository}\n\nFiles: ${loadedFiles.map((f) => f.name).join(", ")}`,
          );
        } else {
          alert(
            `No code files found in the repository. Debug info: ${JSON.stringify(result.debug, null, 2)}`,
          );
        }
      } else {
        console.error("GitHub load error:", result);
        alert(
          `Failed to load files from GitHub: ${result.error || "Unknown error"}`,
        );
      }
    } catch (error) {
      ...
      <div className="h-12 bg-[#2d2d30] border-b border-[#3e3e42] flex items-center justify-between px-4">
        ...
        <div className="flex items-center space-x-3">
          <Button
            ...
            variant="outline"
            size="sm"
            className={`h-7 px-3 text-xs border-[#3e3e42] ${
              isAiEnabled
      ...
      <div className="h-10 bg-[#1e1e1e] border-b border-[#2d2d30] flex items-center justify-between px-4">
        <div className="flex items-center space-x-2 flex-1">
          ...
          <div className="flex items-center space-x-1">
            {openFiles.map((file, index) => (
              <div
                key={index}
                className={`flex items-center space-x-1 px-3 py-1 rounded-t text-sm cursor-pointer ${
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="text-[#FFA500] hover:bg-[#2d2d30] h-7 px-2"
          >
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="text-[#007acc] hover:bg-[#2d2d30] h-7 px-2"
          >
