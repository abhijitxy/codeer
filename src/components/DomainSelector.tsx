...
export function DomainSelector({
  ...
  const { domains: donatedDomains, loading: domainsLoading } =
    useAvailableDomains();
  const {
    checkAvailability,
    getAvailability,
  ...
  useEffect(() => {
    if (
      ...
      checkAvailability(selectedDonatedDomainId, subdomain).then(
        (available) => {
              ...
              {donatedDomains.map((domain) => (
                <div
                  ...
                  className={`border rounded-lg p-4 cursor-pointer transition-all ${
                    selectedDonatedDomainId === domain.id
