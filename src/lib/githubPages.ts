...
export class GitHubPagesAPI {
  ...
  async createRepository(
    ...
    try {
      ...
      if (error.status === 422) {
        throw new Error(
          `Repository name "${repoName}" already exists or is invalid. Please try a different subdomain.`,
        );
      }
      if (error.status === 401) {
  ...
  async uploadFiles(repoName: string, files: FileUpload[]): Promise<boolean> {
    try {
      console.log(
        `Starting file upload for ${files.length} files to ${this.username}/${repoName}`,
      );
      ...
      try {
        console.log(
          `Verifying repository ${this.username}/${repoName} exists...`,
        );
        ...
        throw new Error(
          `Repository ${this.username}/${repoName} not found or not accessible`,
        );
      }
      ...
      try {
        ...
        console.log(
          "Main branch does not exist, will create it with first commit",
        );
      }
      ...
      if (!hasMainBranch) {
        ...
        const indexFile =
          files.find((f) => f.path.toLowerCase() === "index.html") || files[0];

        try {
          console.log(
            `Creating main branch with first file: ${indexFile.path}`,
          );
          ...
          const response =
            await this.octokit.rest.repos.createOrUpdateFileContents({
              ...
              content: Buffer.from(indexFile.content, "utf-8").toString(
                "base64",
              ),
              committer: {
          ...
          console.log(
            `Successfully created main branch and uploaded: ${indexFile.path}`,
          );
          ...
          throw new Error(
            `Failed to initialize repository: ${initialError.message}`,
          );
        }
      }
      ...
      for (const file of files) {
        try {
          console.log(
            `Uploading file: ${file.path} (${file.content.length} characters)`,
          );
          ...
          if (file.path.toLowerCase() === "index.html") {
            throw new Error(
              `Failed to upload critical file ${file.path}: ${fileError.message}`,
            );
          }
        }
      }
      ...
      console.log(
        `Upload summary: ${successfulUploads.length} successful, ${failedUploads.length} failed`,
      );
      ...
      return (
        successfulUploads.length > 0 && successfulUploads.includes("index.html")
      );
    } catch (error: any) {
 ...
 */
export function validateWebsiteFiles(files: FileUpload[]): {
  valid: boolean;
  error?: string;
} {
