  ...
  try {
    ...
    if (fetchError) {
      ...
      if (fetchError.code === "PGRST116") {
        // No user found
        throw new Error(
          "User not found in database. Please sign out and sign in again to create your profile.",
        );
      }
    ...
    throw new Error(
      "User not found in database. Please refresh the page and sign in again.",
    );
  } catch (error) {
...
export const createProject = async (
  ...
  try {
    ...
    if (projectError) {
      ...
      throw new Error(
        `Database error: ${projectError.message || "Unknown error"}`,
      );
    }
...
export const updateProject = async (
  ...
  try {
    ...
    if (fetchError) {
      ...
      throw new Error(
        `Failed to verify project ownership: ${fetchError.message}`,
      );
    }
    ...
    if (updateError) {
      ...
      throw new Error(
        `Database error: ${updateError.message || "Unknown error"}`,
      );
    }
