...
export const createTeamupPost = async (
  formData: TeamupFormData,
  userEmail: string,
  userName?: string,
  userAvatar?: string,
): Promise<TeamupPost> => {
  try {
    ...
    if (formData.teamSlots.length > 0) {
      const { error: slotsError } = await supabase.from("team_slots").insert(
...
// Update TeamUp post
export const updateTeamupPost = async (
  postId: string,
  formData: TeamupFormData,
  userEmail: string,
): Promise<TeamupPost> => {
  try {
    ...
    if (formData.teamSlots && formData.teamSlots.length > 0) {
      ...
      await supabase.from("team_slots").delete().eq("teamup_post_id", postId);
    ...
    if (formData.contactInfo && formData.contactInfo.length > 0) {
      ...
      // Insert new contacts
      const validContacts = formData.contactInfo.filter((contact) =>
        contact.value.trim(),
      );
...
export const getTeamupPosts = async (
  category?: string,
  limit: number = 20,
): Promise<TeamupPost[]> => {
  try {
...
export const getTeamupPostById = async (
  id: string,
): Promise<TeamupPost | null> => {
  try {
...
export const getApplicationsForPost = async (
  postId: string,
): Promise<TeamApplication[]> => {
  try {
    const { data, error } = await supabase
...
// User's TeamUp posts
export const getUserTeamupPosts = async (
  userEmail: string,
): Promise<TeamupPost[]> => {
  try {
    const user = await ensureUserExists(userEmail);
