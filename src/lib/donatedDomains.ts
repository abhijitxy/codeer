...
export class DonatedDomainManager {
  ...
  async reserveSubdomain(
    ...
    try {
      ...
      // Create usage record
      const { error } = await supabase.from("donated_domain_usage").insert({
  ...
  async updateDomainSettings(
    ...
    updates: Partial<
      Pick<
        DonatedDomain,
        "max_subdomains" | "donation_message" | "contact_email" | "terms_of_use"
      >
    >,
  ): Promise<boolean> {
    try {
   ...
   */
  async getDomainCredentials(
    domainId: string,
  ): Promise<{ apiToken: string; zoneId: string } | null> {
    try {
      const { data, error } = await supabase
        .from("donated_domains")
   ...
   */
  private async validateCloudflareCredentials(
    apiToken: string,
    zoneId: string,
  ): Promise<boolean> {
    try {
