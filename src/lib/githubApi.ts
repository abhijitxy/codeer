...
export async function saveProblemToGitHub(
  ...
  try {
    ...
    console.log(
      ...
      accessToken ? accessToken.substring(0, 8) + "..." : "null",
    );
    ...
    if (!testResponse.ok) {
      ...
      if (testResponse.status === 401) {
        throw new Error(
          "Invalid GitHub access token. Please sign out and sign in again.",
        );
        ...
        throw new Error(
          `GitHub API error: ${testResponse.status} ${errorMessage}`,
        );
      }
    }
    ...
    const markdownContent = formatProblemAsMarkdown(
      problemData,
      githubUsername,
    );
    ...
    if (success) {
      try {
        console.log(
          "Saving to fork and creating pull request to main CODEER repository...",
        );
        await saveToForkAndCreatePR(
          accessToken,
          problemData,
          filename,
          markdownContent,
        ...
        console.error(
          "Failed to save to fork or create pull request:",
          prError,
        );
...
function formatProblemAsMarkdown(
  problem: ProblemData,
...
async function ensureRepositoryExists(accessToken: string): Promise<void> {
  try {
    ...
    if (!userResponse.ok) {
      ...
      throw new Error(
        `GitHub API error: ${userResponse.status} ${errorMessage}`,
      );
    }
    ...
    try {
      repoResponse = await fetch(
        `https://api.github.com/repos/${username}/codeer_org_data`,
        {
          headers: {
            Authorization: `token ${accessToken}`,
            Accept: "application/vnd.github.v3+json",
    ...
    if (repoResponse.status === 404) {
      ...
      throw new Error(
        `GitHub API error: ${repoResponse.status} ${errorMessage}`,
      );
    } else {
...
async function createFileInRepository(
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Network error getting user for file creation:",
        fetchError,
      );
      return false;
    }
    ...
    if (!createResponse.ok) {
      ...
      console.error(
        `File creation failed: ${createResponse.status} ${errorMessage}`,
      );
      return false;
    }
...
async function saveToForkAndCreatePR(
  ...
  try {
    ...
    const mainRepoResponse = await fetch(
      `https://api.github.com/repos/${mainRepoOwner}/${mainRepoName}`,
      {
        headers: {
          Authorization: `token ${accessToken}`,
          Accept: "application/vnd.github.v3+json",
    ...
    if (!mainRepoResponse.ok) {
      throw new Error(
        `Main repository ${mainRepoOwner}/${mainRepoName} does not exist or is not accessible`,
      );
    }
    ...
    try {
      const forkCheckResponse = await fetch(
        `https://api.github.com/repos/${username}/${mainRepoName}`,
        {
          headers: {
            Authorization: `token ${accessToken}`,
            Accept: "application/vnd.github.v3+json",
      ...
      if (forkCheckResponse.ok) {
        ...
        if (
          forkRepo.fork &&
          forkRepo.parent &&
          forkRepo.parent.full_name === `${mainRepoOwner}/${mainRepoName}`
        ) {
          forkExists = true;
          console.log("✅ Fork already exists");
        } else {
          console.log(
            "❌ Repository exists but is not a fork of the main repo",
          );
          throw new Error(
            `Repository ${username}/${mainRepoName} exists but is not a fork of ${mainRepoOwner}/${mainRepoName}`,
          );
        }
      }
    } catch (error) {
    ...
    if (!forkExists) {
      ...
      const forkResponse = await fetch(
        `https://api.github.com/repos/${mainRepoOwner}/${mainRepoName}/forks`,
        {
          method: "POST",
          headers: {
            Authorization: `token ${accessToken}`,
      ...
      if (!forkResponse.ok) {
        ...
        throw new Error(
          `Failed to create fork: ${forkResponse.status} ${errorData.message || ""}`,
        );
      }
      ...
      while (retries > 0) {
        try {
          ...
          console.log(
            `⏳ Fork not ready yet, retrying... (${retries} attempts left)`,
          );
        }
    ...
    try {
      const existingFileResponse = await fetch(
        `https://api.github.com/repos/${username}/${mainRepoName}/contents/${filePath}`,
        {
          headers: {
            Authorization: `token ${accessToken}`,
            Accept: "application/vnd.github.v3+json",
    ...
    if (!createFileResponse.ok) {
      ...
      throw new Error(
        `Failed to create file in fork: ${createFileResponse.status} ${errorData.message || ""}`,
      );
    }
...
> This problem was automatically submitted through the CODEER Platform's community contribution system.`;
    ...
    const prResponse = await fetch(
      `https://api.github.com/repos/${mainRepoOwner}/${mainRepoName}/pulls`,
      {
        method: "POST",
        headers: {
          Authorization: `token ${accessToken}`,
    ...
    if (!prResponse.ok) {
      ...
      if (prResponse.status === 422 && errorData.errors) {
        const alreadyExistsError = errorData.errors.find(
          (err: any) =>
      ...
      throw new Error(
        `Failed to create pull request: ${prResponse.status} ${errorData.message || "Unknown error"}`,
      );
    }
