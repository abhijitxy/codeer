...
export class CloudflareAPI {
  ...
  async createSubdomain(
    ...
    try {
      const domainConfig =
        DOMAIN_CONFIGS[domain as keyof typeof DOMAIN_CONFIGS];
      if (!domainConfig) {
      ...
      if (response.success) {
        console.log(
          `DNS record created successfully for ${subdomain}.${domain}`,
        );
        return true;
      } else {
    ...
    try {
      ...
      if (recordsResponse.success && recordsResponse.result.length > 0) {
        ...
        if (deleteResponse.success) {
          console.log(
            `DNS record deleted successfully for ${subdomain}.${domain}`,
          );
          return true;
        } else {
  ...
  async isSubdomainAvailable(
    ...
    try {
      ...
      console.error(
        `Error checking subdomain availability for ${subdomain}.${domain}:`,
        error,
      );
...
export class DonatedDomainCloudflare {
  ...
  static async isSubdomainAvailableOnDonatedDomain(
    ...
    try {
      ...
      console.error(
        "Error checking subdomain availability on donated domain:",
        error,
      );
      return false;
    }
...
export async function checkSubdomainAvailability(subdomain: string): Promise<{
  ...
  const dbTakenDomains = new Set(
    existingPages?.map((page) => page.domain) || [],
  );
  ...
  for (const domain of domains) {
    if (dbTakenDomains.has(domain)) {
      ...
      results[domain] = await cloudflare.isSubdomainAvailable(
        subdomain,
        domain,
      );
