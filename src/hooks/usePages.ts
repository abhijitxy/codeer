...
export function useCreatePage() {
  ...
  const createPage = useCallback(
    async (formData: CreatePageForm): Promise<PageCreationResult> => {
  ...
  const checkAvailability = useCallback(async (subdomain: string) => {
    ...
    try {
      const response = await fetch(
        `/api/pages/check-availability?subdomain=${encodeURIComponent(subdomain)}`,
      );
...
export function usePageManagement() {
  ...
  const updatePage = useCallback(
    async (pageId: string, files: FileList): Promise<boolean> => {
